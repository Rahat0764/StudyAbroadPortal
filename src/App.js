...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError)
      return (
        <div className="p-10 text-center text-red-500 bg-[#070b12] min-h-screen">
...
try {
  ...
  console.warn("Firebase config issue, analytics disabled.");
}
...
const CacheService = {
  ...
  get(key) {
    return this.cache.get(key.toLowerCase().trim());
  },
};
...
function MarkdownRenderer({ text }) {
  ...
  const parseInline = (str) => {
    ...
    safeHtml = safeHtml.replace(
      /\*(.*?)\*/g,
      '<em class="text-[#22c7b8]">$1</em>'
    );
    safeHtml = safeHtml.replace(
      /`(.*?)`/g,
      '<code class="bg-[#1a2a3a] text-[#4a9eff] px-1.5 py-0.5 rounded text-sm">$1</code>'
    );
    safeHtml = safeHtml.replace(
      /\[([^\]]+)\]\(([^)]+)\)/g,
      (match, title, url) => {
        return `<a href="${sanitizeUrl(
          url
        )}" target="_blank" rel="noopener noreferrer" class="text-[#4a9eff] hover:underline hover:text-[#2ecc8a] font-medium break-all">${title}</a>`;
      }
    );
    return safeHtml;
  };
      ...
      if (listType === "ul")
        elements.push(
          <ul
            ...
            className="list-disc list-inside mb-4 space-y-1 text-[#7a94ad]"
          >
            {currentList}
          </ul>
        );
        ...
        elements.push(
          <ol
            ...
            className="list-decimal list-inside mb-4 space-y-1 text-[#7a94ad]"
          >
            {currentList}
          </ol>
        );
  ...
  lines.forEach((line, index) => {
    ...
    if (!trimmed) elements.push(<div key={`br-${index}`} className="h-3" />);
      ...
      elements.push(
        <h3
          key={`h3-${index}`}
          className="text-xl font-bold text-[#d4a843] mt-6 mb-3 border-b border-[#141f2e] pb-2"
        >
      ...
      elements.push(
        <h2
          key={`h2-${index}`}
          className="text-2xl font-extrabold text-white mt-8 mb-4 bg-gradient-to-r from-[#f0c96622] to-transparent p-3 rounded-lg border-l-4 border-[#d4a843]"
        >
      ...
      elements.push(
        <h1
          key={`h1-${index}`}
          className="text-3xl font-extrabold text-[#d4a843] mt-4 mb-6"
        >
      ...
      elements.push(
        <p
          key={`p-${index}`}
          className="text-[#7a94ad] leading-relaxed mb-3"
  ...
  return (
    <div className="animate-pulse space-y-6 pt-4">
      ...
      <div className="space-y-3">
        <div className="h-4 bg-[#141f2e] rounded w-full"></div>
        <div className="h-4 bg-[#141f2e] rounded w-5/6"></div>
        <div className="h-4 bg-[#141f2e] rounded w-4/6"></div>
      </div>
    </div>
  );
}
...
function MainApp() {
  ...
  // Load Search History from Local Storage (Fix Medium 17)
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    fetchUserIPDetails().then(setUserInfo);
  }, []);
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((doc) =>
          data.push({ id: doc.id, count: doc.data().count || 0 })
        );
  ...
  const updateAnalytics = useCallback(
    async (countryName) => {
      ...
      try {
        ...
        console.warn("Analytics error suppressed");
      }
    },
    [userAuth]
  );
  ...
  const fetchScholarshipDetails = useCallback(
    async (country, lvl, bg) => {
      if (isOffline)
        return setError("You are offline. Please check connection.");
      ...
      const cacheKey = `C_${country.name}_L_${lvl}_B_${bg}`;
      const cachedData = CacheService.get(cacheKey);
      if (cachedData) {
        setResultText(cachedData);
        setLoading(false);
        return;
      }

      const prompt = `Act as an international scholarship consultant for Bangladeshi students. 
    Provide comprehensive information about scholarships in ${country.name}.
    Target Level: ${lvl} | Background: ${bg}
    IMPORTANT: Use Google Search to find exact, official application portal URLs.
      ...
      try {
        ...
        setError(
          err.message.includes("Unexpected token")
            ? "⚠️ Backend API not found. Please deploy to Vercel."
            : err.message
        );
      } finally {
        setLoading(false);
    ...
    [userInfo, updateAnalytics, isOffline]
  );

  const handleGlobalAsk = useCallback(
    async (queryOverride) => {
      const queryToUse =
        typeof queryOverride === "string" ? queryOverride : globalAskQuery;
      if (!queryToUse.trim() || isOffline) return;

      setLoading(true);
      setGlobalAskResult(null);
      setView(VIEWS.SEARCH);
      setCopied(false);

      const cacheKey = `Global_${queryToUse.trim()}`;
      const cachedData = CacheService.get(cacheKey);
      if (cachedData) {
        setGlobalAskResult(cachedData);
        setLoading(false);
    ...
    Use Google Search to find accurate information and verified links.`;
      ...
      try {
        ...
        setGlobalAskResult("Error: " + err.message);
      } finally {
        setLoading(false);
      }
    },
    [globalAskQuery, userInfo, isOffline]
  );

  return (
    <div className="min-h-screen bg-[#03050a] text-[#dde6f0] font-sans selection:bg-[#d4a843] selection:text-black pb-12">
      {isOffline && (
        <div className="bg-[#e05555] text-white text-center py-1 text-sm font-bold">
      ...
      <header className="sticky top-0 z-50 bg-[#050810]/90 backdrop-blur-md border-b border-[#141f2e]">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#d4a843] to-[#8a6b24] flex items-center justify-center text-xl shadow-lg">
              🎓
            </div>
            <div>
              <h1 className="text-xl font-bold leading-tight font-serif text-white tracking-wide">
                ScholarPath
              </h1>
              <p className="text-[10px] text-[#d4a843] tracking-widest font-semibold uppercase">
                Optimized Edition
              </p>
            </div>
          </div>
          <div className="flex bg-[#0b1119] p-1 rounded-lg border border-[#141f2e]">
            <button
              aria-label="View Countries"
              onClick={() => setView(VIEWS.COUNTRIES)}
              className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${
                view === VIEWS.COUNTRIES || view === VIEWS.RESULT
                  ? "bg-[#d4a843] text-black shadow-sm"
                  : "text-[#7a94ad] hover:text-white"
              }`}
            >
              🌍 Countries
            </button>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
            ...
            <div className="mb-10 flex flex-col md:flex-row gap-8 justify-between items-start">
              <div className="text-left flex-1">
                ...
                <p className="text-[#7a94ad] max-w-2xl text-base md:text-lg">
                  AI ইন্টারনেট সার্চ করে আসল ও ভেরিফায়েড স্কলারশিপের তথ্য বের
                  করবে।
                </p>
              </div>
              {analyticsData.length > 0 && (
                <div className="bg-[#070b12] border border-[#141f2e] rounded-2xl p-4 w-full md:w-64 flex-shrink-0 shadow-lg">
                  <h3 className="text-[#d4a843] font-bold text-sm mb-3">
                    🔥 Trending Searches
                  </h3>
                  <div className="space-y-2">
                    {analyticsData.slice(0, 5).map((item, idx) => (
                      <div
                        key={item.id}
                        className="flex justify-between items-center bg-[#0b1119] p-2 rounded-lg border border-[#141f2e]"
                      >
                        <span className="text-xs text-white">
                          <span className="text-[#7a94ad] mr-1">
                            #{idx + 1}
                          </span>{" "}
                          {item.id}
                        </span>
                        <span className="text-xs bg-[#141f2e] text-[#4a9eff] px-2 py-0.5 rounded-full font-bold">
                          {item.count}
                        </span>
                      </div>
                    ))}
            ...
            <div className="bg-[#070b12] border border-[#141f2e] rounded-2xl p-5 mb-8 flex flex-col md:flex-row gap-6 shadow-lg">
              <div className="flex-1">
                <label
                  ...
                  className="block text-xs font-bold text-[#3d5269] uppercase mb-2"
                >
                  Search Country
                </label>
                <input
                  ...
                  className="w-full bg-[#0b1119] border border-[#141f2e] focus:border-[#d4a843] rounded-xl px-4 py-3 text-white outline-none transition-all placeholder-[#3d5269]"
                />
              </div>
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-[#3d5269] uppercase mb-2">
                    Degree Level
                  </label>
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-full text-xs font-semibold capitalize transition-all ${
                          level === l
                            ...
                            : "bg-[#141f2e]/50 text-[#7a94ad] hover:bg-[#141f2e]"
                        }`}
                      >
                        {l}
                      </button>
                ...
                {filteredCountries.map((c) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      fetchScholarshipDetails(c, level, background);
                    }}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        setSelectedCountry(c);
                    ...
                    className="bg-[#0b1119] border border-[#141f2e] hover:border-[#d4a843]/60 rounded-2xl p-4 cursor-pointer transition-all hover:-translate-y-1 hover:shadow-lg focus:outline-none focus:border-[#d4a843] group"
                  >
                    ...
                    <p className="text-xs text-[#3d5269] line-clamp-2">
                      {c.hint}
                    </p>
                  </div>
                ))}
        ...
        {view === VIEWS.RESULT && (
          <div className="animate-in fade-in max-w-4xl mx-auto">
            <button
              onClick={() => setView(VIEWS.COUNTRIES)}
              className="flex items-center gap-2 text-[#7a94ad] hover:text-white mb-6 transition-colors px-3 py-1.5 rounded-lg hover:bg-[#141f2e]"
            >
              &larr; Back
            </button>
            <div className="bg-[#070b12] border border-[#141f2e] rounded-3xl p-6 md:p-10 shadow-2xl relative">
              <div className="flex flex-col md:flex-row justify-between gap-6 mb-8 pb-8 border-b border-[#141f2e]">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-3xl md:text-4xl font-serif font-bold text-white mb-1">
                      {selectedCountry?.name}
                    </h2>
                    <p className="text-sm text-[#d4a843] font-medium tracking-wide">
                      VERIFIED REPORT (Lvl: {level}, Bg: {background})
                    </p>
                  </div>
                </div>
                {resultText && !loading && (
                  <button
                    ...
                    className="px-4 py-2 bg-[#141f2e] hover:bg-[#1e3045] text-white rounded-lg text-sm font-medium transition-colors"
                  >
                    {copied ? "✅ Copied!" : "📋 Copy"}
                  </button>
                )}
              ...
              {loading ? (
                ...
                <div className="bg-[#e05555]/10 border border-[#e05555]/30 rounded-xl p-6 text-[#e05555]">
                  ...
                  <button
                    ...
                    className="mt-4 px-4 py-2 bg-[#e05555]/20 hover:bg-[#e05555]/30 rounded-lg text-sm"
                  >
                    Retry
                  </button>
                </div>
        ...
        {view === VIEWS.SEARCH && (
          <div className="animate-in fade-in max-w-3xl mx-auto">
            <div className="mb-8">
              ...
              <p className="text-[#7a94ad]">
                স্কলারশিপ নিয়ে যেকোনো প্রশ্ন করুন (গুগল সার্চ সাপোর্টেড)।
              </p>
            </div>

            {searchHistory.length > 0 && (
              <div className="mb-6 flex gap-2 flex-wrap">
                <span className="text-xs text-[#3d5269] self-center mr-2">
                  Recent:
                </span>
                {searchHistory.map((h, i) => (
                  <button
                    ...
                    className="text-xs bg-[#141f2e] text-[#7a94ad] hover:text-white px-3 py-1.5 rounded-full transition-colors"
                  >
                    {h}
                  </button>
                ))}
            ...
            <div className="relative mb-8">
              <textarea
                ...
                value={globalAskQuery}
                onChange={(e) => setGlobalAskQuery(e.target.value)}
                onKeyDown={(e) => {
                ...
                placeholder="Ex: Suggest fully funded bachelor scholarships in Europe..."
                className="w-full bg-[#070b12] border border-[#141f2e] focus:border-[#d4a843] rounded-2xl p-5 pr-24 text-white outline-none resize-none min-h-[120px]"
              />
              <button
                onClick={() => handleGlobalAsk()}
                disabled={loading || !globalAskQuery.trim()}
                className="absolute bottom-4 right-4 bg-[#d4a843] hover:bg-[#e5b954] text-black font-bold px-6 py-2.5 rounded-xl disabled:opacity-50"
              >
              ...
              globalAskResult && (
                <div className="bg-[#070b12] border border-[#141f2e] rounded-2xl p-6 md:p-8 relative shadow-xl">
                  {!globalAskResult.startsWith("Error:") && (
                    <button
                      onClick={() => handleCopy(globalAskResult)}
                      className="absolute top-4 right-4 px-3 py-1 bg-[#141f2e] text-[#7a94ad] hover:text-white rounded-md text-xs transition-colors"
                    >
                      {copied ? "Copied" : "Copy"}
                    </button>
                  )}
                  {globalAskResult.startsWith("Error:") ? (
                    <p className="text-[#e05555]">{globalAskResult}</p>
                  ) : (
                    <MarkdownRenderer text={globalAskResult} />
                  )}
                </div>
              )
            )}
