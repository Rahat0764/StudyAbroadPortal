...
By using BideshPro, you agree to these Terms & Conditions.

## 2. Accuracy Disclaimer
⚠️ **IMPORTANT:** All scholarship information provided by BideshPro is AI-generated from web searches. Always verify information directly from official scholarship websites before applying.
`;
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-[#070b12] border border-[#1e3045] rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl">
        ...
        <div className="px-6 py-4 border-t border-[#141f2e] flex-shrink-0">
          <button onClick={onClose} className="w-full bg-[#d4a843] text-black font-bold py-2.5 rounded-xl hover:bg-[#e5b954] transition-colors">
            I Understand — Close
          </button>
        </div>
      </div>
    ...
    <div className="fixed bottom-0 left-0 right-0 z-[100] bg-[#070b12] border-t border-[#1e3045] shadow-2xl p-4">
      <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <div className="flex-1 text-sm text-[#7a94ad]">
          🍪 BideshPro collects anonymous usage data for analytics. {" "}
          <button onClick={onViewPolicy} className="text-[#4a9eff] underline hover:text-[#2ecc8a]">Privacy Policy</button>
        </div>
        <button onClick={onAccept} className="flex-shrink-0 bg-[#d4a843] text-black font-bold px-6 py-2 rounded-xl text-sm hover:bg-[#e5b954] transition-colors">
          Accept & Continue
        </button>
      </div>
    </div>
...
- 🗓 **Intake:** ...
- ⏳ **Duration:** ...
- ✅ **Eligibility:** (SSC/HSC/CGPA/Background)
- 📄 **Required Documents:** ...
- 🗣 **Language:** ...
- 🇧🇩 **Bangladesh Quota:** ...
...
## 🔗 All Important Links`;
  ...
  const fetchScholarship = useCallback(
    async (country, lvl, bg) => {
      ...
      try {
        ...
        setError(err.message.includes("not found") ? "⚠️ Backend API not connected." : err.message);
  ...
  const handleGlobalAsk = useCallback(
    async (qOverride) => {
      ...
      const prompt = `You are an expert international scholarship and study abroad consultant for Bangladeshi students. Answer this question comprehensively with verified, current information: "${q}"
      ⚠️ Rules: Write entirely in ${language}, Include REAL URLs.`;
  ...
  return (
    <div className="min-h-screen bg-[#03050a] text-[#dde6f0] font-sans selection:bg-[#d4a843] selection:text-black pb-20">
      {isOffline && <div className="bg-red-600 text-white text-center py-1.5 text-sm font-semibold tracking-wide">⚡ You are offline — results from cache only</div>}

      <header className="sticky top-0 z-50 bg-[#050810]/95 backdrop-blur-md border-b border-[#141f2e]">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 sm:gap-3 cursor-pointer flex-shrink-0" onClick={() => { setView(VIEWS.COUNTRIES); setSelectedCountry(null); }}>
            ...
            <div className="flex flex-col">
              ...
              <p className="text-[8px] sm:text-[10px] text-[#7a94ad] tracking-widest font-semibold uppercase mt-0.5 flex items-center">
                ...
                Developed by <a href="https://www.linkedin.com/in/RahatAhmedX" target="_blank" rel="noopener noreferrer" className="text-[#4a9eff] hover:underline ml-0.5">Rahat</a>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            <div className="flex bg-[#141f2e] rounded-lg p-0.5 border border-[#1e3045]">
              {["English", "Bengali"].map((lang) => (
                <button key={lang} onClick={() => setLanguage(lang)} className={`px-2 py-1 sm:px-2.5 text-[10px] sm:text-[11px] font-bold rounded-md transition-all ${language === lang ? "bg-[#d4a843] text-black" : "text-[#7a94ad] hover:text-white"}`}>
                  {lang === "English" ? "EN" : "বাং"}
                </button>
              ))}
            </div>

            {[ { v: VIEWS.COUNTRIES, icon: "🌍", label: "Countries" }, { v: VIEWS.SEARCH, icon: "🔍", label: "Ask AI" }].map(({ v, icon, label }) => (
              <button key={v} onClick={() => setView(v)} className={`px-2.5 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all ${view === v || (v === VIEWS.COUNTRIES && view === VIEWS.RESULT) ? "bg-[#d4a843] text-black shadow-sm" : "text-[#7a94ad] hover:text-white hover:bg-[#141f2e]"}`}>
                <span>{icon}</span><span className="hidden sm:inline ml-1">{label}</span>
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="animate-in fade-in duration-300">
            <div className="mb-10">
              <h2 className="text-3xl md:text-5xl font-serif font-bold text-white mb-3 leading-tight">
                Study Abroad Scholarships<br /><span className="text-[#d4a843]">for Bangladeshi Students</span>
              </h2>
            </div>
            ...
            {analyticsData.length > 0 && (
              <div className="flex items-center gap-2 mb-5 flex-wrap">
                ...
                {analyticsData.slice(0, 5).map((a) => (
                  <button key={a.id} onClick={() => { const c = COUNTRIES.find((x) => x.name === a.id); if (c) { setSelectedCountry(c); fetchScholarship(c, level, background); } }} className="text-xs bg-[#141f2e] hover:bg-[#1e3045] text-[#d4a843] px-3 py-1 rounded-full border border-[#1e3045] transition-colors">
                    {COUNTRIES.find((c) => c.name === a.id)?.flag || "🌍"} {a.id} <span className="ml-1 text-[#3d5269]">×{a.count}</span>
                  </button>
                ))}
              </div>
            )}
            ...
            <div className="bg-[#070b12] border border-[#141f2e] rounded-2xl p-5 mb-8 flex flex-col md:flex-row gap-5">
              <div className="flex-1">
                ...
                <input type="text" placeholder="e.g. Germany, Japan..." value={countrySearch} onChange={(e) => setCountrySearch(e.target.value)} className="w-full bg-[#0b1119] border border-[#141f2e] focus:border-[#d4a843] rounded-xl px-4 py-3 text-white outline-none transition-all placeholder-[#3d5269] text-sm" />
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["all", "bachelor", "masters", "phd"].map((l) => (
                      <button key={l} onClick={() => setLevel(l)} className={`px-4 py-1.5 rounded-full text-xs font-semibold capitalize transition-all ${level === l ? "bg-[#d4a843] text-black" : "bg-[#141f2e]/60 text-[#7a94ad] hover:bg-[#141f2e]"}`}>{l === "all" ? "All Levels" : l === "phd" ? "PhD" : l.charAt(0).toUpperCase() + l.slice(1)}</button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["all", "science", "arts", "commerce"].map((bg) => (
                      <button key={bg} onClick={() => setBackground(bg)} className={`px-4 py-1.5 rounded-full text-xs font-semibold capitalize transition-all ${background === bg ? "bg-[#d4a843] text-black" : "bg-[#141f2e]/60 text-[#7a94ad] hover:bg-[#141f2e]"}`}>{bg === "all" ? "All" : bg.charAt(0).toUpperCase() + bg.slice(1)}</button>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
              {filteredCountries.map((c) => {
                ...
                return (
                  <div key={c.name} tabIndex={0} role="button" onClick={() => { setSelectedCountry(c); fetchScholarship(c, level, background); }} className="relative bg-[#0b1119] border border-[#141f2e] hover:border-[#d4a843]/60 rounded-2xl p-4 cursor-pointer transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-[#d4a843]/5 focus:outline-none focus:border-[#d4a843] group">
          ...
          <div className="animate-in fade-in max-w-4xl mx-auto">
            <div className="flex items-center gap-3 mb-6 flex-wrap">
              ...
              <div className="ml-auto flex gap-1.5 flex-wrap">
                {["all", "bachelor", "masters", "phd"].map((l) => (
                  <button key={l} onClick={() => { setLevel(l); fetchScholarship(selectedCountry, l, background); }} className={`px-3 py-1 rounded-full text-xs font-semibold capitalize transition-all ${level === l ? "bg-[#d4a843] text-black" : "bg-[#141f2e] text-[#7a94ad] hover:text-white"}`}>{l === "all" ? "All" : l === "phd" ? "PhD" : l.charAt(0).toUpperCase() + l.slice(1)}</button>
                ))}
              </div>
            </div>
            ...
            <div ref={resultRef} className="bg-[#070b12] border border-[#141f2e] rounded-3xl p-6 md:p-10 shadow-2xl">
              <div className="flex flex-col md:flex-row justify-between gap-5 mb-8 pb-6 border-b border-[#141f2e]">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#d4a843] font-medium tracking-wide mt-1">AI VERIFIED REPORT · Level: {level.toUpperCase()} · Background: {background.toUpperCase()}</p>
                  </div>
                </div>
                {resultText && !loading && (
                  <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => handleCopy(resultText)} className="px-4 py-2 bg-[#141f2e] hover:bg-[#1e3045] text-white rounded-xl text-sm font-medium transition-all">{copied ? "✅ Copied!" : "📋 Copy"}</button>
                  </div>
                )}
              </div>
              ...
              {loading ? (
                ...
                <div className="bg-[#1a0a0a] border border-[#e05555]/30 rounded-2xl p-6">
                  <h4 className="font-bold text-[#e05555] mb-2">⚠️ Error</h4><p className="text-sm text-[#e05555]/80 mb-4">{error}</p>
                ...
                <>
                  ...
                  <div className="mt-8 p-4 bg-[#d4a843]/8 border border-[#d4a843]/25 rounded-xl text-xs text-[#7a94ad]">⚠️ Always verify deadlines and requirements from official scholarship websites.</div>
                </>
              ) : null}
            </div>
          </div>
          ...
          <div className="animate-in fade-in max-w-3xl mx-auto">
            ...
            {history.length > 0 && (
              <div className="flex gap-2 flex-wrap mb-4 items-center">
                ...
                {history.map((h, i) => (
                  <button key={i} onClick={() => { setGlobalQ(h); handleGlobalAsk(h); }} className="text-xs bg-[#141f2e] hover:bg-[#1e3045] text-[#7a94ad] hover:text-white px-3 py-1.5 rounded-full border border-[#1e3045] truncate max-w-[180px]">{h}</button>
                ))}
              </div>
            )}

            <div className="relative mb-6">
              <textarea value={globalQ} onChange={(e) => setGlobalQ(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleGlobalAsk(); } }} placeholder="Ask anything about study abroad..." className="w-full bg-[#070b12] border border-[#141f2e] focus:border-[#d4a843] rounded-2xl p-5 pr-28 text-white outline-none resize-none min-h-[120px] text-sm transition-all" />
              <button onClick={() => handleGlobalAsk()} disabled={loading || !globalQ.trim()} className="absolute bottom-4 right-4 bg-[#d4a843] hover:bg-[#e5b954] disabled:opacity-40 text-black font-bold px-5 py-2.5 rounded-xl text-sm transition-all">{loading ? "Thinking…" : "Ask →"}</button>
            </div>

            {loading ? <div className="bg-[#070b12] rounded-2xl p-8 border border-[#141f2e]"><SkeletonLoader /></div> : globalResult ? (
              <div className="bg-[#070b12] border border-[#141f2e] rounded-2xl p-6 md:p-8 relative shadow-xl">
                {!globalResult.startsWith("❌") && <button onClick={() => handleCopy(globalResult)} className="absolute top-4 right-4 px-3 py-1 bg-[#141f2e] text-[#7a94ad] hover:text-white rounded-lg text-xs">{copied ? "✅" : "📋 Copy"}</button>}
                {globalResult.startsWith("❌") ? <p className="text-[#e05555] text-sm">{globalResult}</p> : <MarkdownRenderer text={globalResult} />}
              </div>
            ) : null}
          </div>
        )}
      ...
      <footer className="border-t border-[#141f2e] mt-12 py-8 px-4"><div className="max-w-6xl mx-auto text-xs text-[#3d5269] text-center">BideshPro © 2025</div></footer>
    </div>
  );
}
